...
data_root = 'data/skypool_first_trainval/'
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)
train_pipeline = [
    dict(type='LoadImageFromFile'),
    dict(type='LoadAnnotations', with_bbox=True),
    dict(type='Resize', img_scale=(2446, 1000), keep_ratio=True),  # 考虑算力原有限，修改图像尺寸为半图，可修改为全图训练
...
data = dict(
    ...
    train=dict(
        ...
        img_prefix=data_root + 'train/',  # 训练图片路径
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/instances_val.json',  # 修改成自己的验证集标注文件路径
        img_prefix=data_root + 'val/',  # 验证图片路径
        pipeline=test_pipeline),
    test=dict(
        type=dataset_type,
        ann_file=data_root + 'annotations/instances_val.json',  # 修改成自己的验证集标注文件路径
        img_prefix=data_root + 'val/',  # 测试图片路径
        pipeline=test_pipeline))

# optimizer
optimizer = dict(type='SGD', lr=0.005, momentum=0.9, weight_decay=0.0001)  # 学习率的设置尤为关键：lr = 0.00125*batch_size
...
log_level = 'INFO'
work_dir = './submit/train'  # 训练的权重和日志保存路径
load_from = './mmdetection/checkpoints/cascade_rcnn_r50_coco_pretrained_weights_classes_21.pth'  # 采用coco预训练模型 ,需要对权重类别数进行处理
resume_from = None
workflow = [('train', 1)]
